...
/*>>>*/
	function setup ($set) { /*<<<*/
		...
		if (array_key_exists("scolors", $this->options) && is_array($this->options["scolors"])) {
...
/*>>>*/
	function render_label($blx, $bly, $font, $size, $color, $text, $opts="") {/*<<<*/
		...
		*/
		...
		if ($bly > $this->options["height"]) {
			$this->log("cannot render label \"$text\" at y = $bly");
			return;
		}
		if (isset($this->robjects) && is_array($this->robjects)) {
		...
		if ($i < 10) {
			if (array_key_exists("shadow", $options) && $options["shadow"]) {
...
/*>>>*/
	function render_axes() {/*<<<*/
		...
		$absolute_max_x = 
			($this->legend_x0 - $fw - (15 * $this->options["padding"]));
		...
		if ($this->options["type"] == "bar") {
			...
			if ($this->x_numeric) {
				...
				//	extreme left.
				$this->min_scale_x = 0;
				// 2005-10-17 :: fix supplied by Ulla Küchler for non-numeric
				//	x scale
				//$this->max_scale_x = $this->max_x;
				//$this->xtranspx = $this->graph_w / $this->max_scale_x;
				$this->max_scale_x = $this->xlabels[$this->max_x];
				$this->xtranspx = $this->graph_w / $this->max_x;
		...
		// 2005-10-17 :: change, thanks to Ulla Küchler (unverified) 
		//	-- fixes for non-numeric x labels
			$last_mark = array_search($last_mark, $this->xlabels);
		}
      ...
      if (array_key_exists($i, $this->xlabels))
...
/*>>>*/
	function render_vbars() {/*<<<*/
		...
		$this->bar_width = $this->x_marker_distance / 
			...
			- (2 * $this->options["padding"]);
		$this->log("bar_width is ".$this->bar_width);
		$sidx = 0;
...
/*>>>*/
    ...
    if (isset($this->xlabels) &&
        is_array($this->xlabels) && 
        count($this->xlabels)) {
			$labels = implode(",", $this->xlabels);
			$this->tack_arg($url, "xl", $labels);
    }
    if (isset($this->ylabels) && 
        is_array($this->ylabels) && 
        count($this->ylabels)) {
